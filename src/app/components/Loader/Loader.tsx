...
export const CyberpunkLoader = ({ onLoadingComplete }: { onLoadingComplete: () => void }) => {
  ...
  return (
    <div className={`fixed inset-0 bg-black z-50 flex flex-col items-center justify-center transition-opacity duration-800 ${isFading ? 'opacity-0' : 'opacity-100'}`}>
      ...
      {/* Percentage display - responsive text size */}
      <div className="text-white text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-mono font-bold mb-4 sm:mb-6 md:mb-8 relative z-10">
        {Math.min(progress, 100).toFixed(0)}%
      </div>

      {/* Progress bar - responsive width and margins */}
      <div className="w-[90%] sm:w-[80%] md:max-w-xl relative z-10 group px-4">
        <div className="h-1 sm:h-1.5 bg-white bg-opacity-10 mb-3 sm:mb-4 overflow-hidden rounded-full">
          <div 
        ...
        <div className="text-[10px] xs:text-xs text-white text-opacity-50 font-mono text-right -mt-2 sm:-mt-3 transition-opacity group-hover:opacity-100 opacity-0">
          {Math.min(progress, 100).toFixed(1)}% LOADED
        </div>
      </div>

      {/* Bottom-right text - responsive size and positioning */}
      <div className="absolute bottom-3 right-3 text-white text-opacity-70 font-mono text-xl xs:text-2xl sm:text-3xl md:text-4xl lg:text-5xl tracking-widest animate-pulse">
        ACCESSING LABS...
      </div>
    </div>
  );
