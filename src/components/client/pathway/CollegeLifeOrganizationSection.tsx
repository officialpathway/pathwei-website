    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-3 text-white">
        <div className="flex justify-between items-center">
          ...
          <div className="flex space-x-1">
            <button title='not-usable' type='button' className="p-1 hover:bg-white/20 rounded">
      ...
      <div className="p-2">
        <div className="grid grid-cols-7 gap-0.5">
          ...
          {[28, 29, 30, 1, 2, 3, 4].map((day, i) => (
            <div 
              key={`week1-${i}`} 
              className={`aspect-square p-0.5 rounded ${i < 3 ? 'text-gray-400' : ''}`}
            >
                ...
                {[1, 3, 5].includes(i) && (
                  <div className="mt-auto mb-0.5">
                    <div className={`h-1 w-full rounded-full ${
                      i === 1 ? 'bg-amber-300' : i === 3 ? 'bg-green-300' : 'bg-purple-300'
                    }`}></div>
                  </div>
                )}
              </div>
          ...
          {[12, 13, 14, 15, 16, 17, 18].map((day, i) => (
            <div 
              key={`week3-${i}`} 
              className={`aspect-square p-0.5 rounded ${day === 13 ? 'bg-blue-100 font-bold text-blue-800' : ''}`}
            >
              <div className="h-full w-full flex flex-col">
                <span className="text-xs">{day}</span>
                {i === 1 && (
                ...
                {[0, 4].includes(i) && (
                  <div className="mt-auto mb-0.5">
                    <div className={`h-1 w-full rounded-full ${i === 0 ? 'bg-green-300' : 'bg-red-300'}`}></div>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="p-3 border-t border-gray-100">
        <div className="flex items-center justify-between mb-2">
          <h4 className="font-medium text-gray-800 text-sm">{t('calendar.today_label')}</h4>
          <span className="text-xs font-medium px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full">{t('calendar.activities_count')}</span>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center p-1.5 bg-green-50 rounded border-l-3 border-green-500">
            <div className="w-10 flex-shrink-0">
              <span className="text-xs font-medium text-green-800">{t('daily_schedule.0.time')}</span>
            </div>
            <div className="flex-grow">
              <h5 className="text-xs font-medium text-gray-800">{t('daily_schedule.0.activity')}</h5>
            </div>
          ...
          <div className="flex items-center p-1.5 bg-blue-50 rounded border-l-3 border-blue-500">
            <div className="w-10 flex-shrink-0">
              <span className="text-xs font-medium text-blue-800">{t('daily_schedule.1.time')}</span>
            </div>
            <div className="flex-grow">
              <h5 className="text-xs font-medium text-gray-800">{t('daily_schedule.1.activity')}</h5>
            </div>
          ...
          <div className="flex items-center p-1.5 bg-amber-50 rounded border-l-3 border-amber-500">
            <div className="w-10 flex-shrink-0">
              <span className="text-xs font-medium text-amber-800">{t('daily_schedule.2.time')}</span>
            </div>
            <div className="flex-grow">
              <h5 className="text-xs font-medium text-gray-800">{t('daily_schedule.2.activity')}</h5>
            </div>
          ...
          <div className="flex items-center p-1.5 bg-purple-50 rounded border-l-3 border-purple-500">
            <div className="w-10 flex-shrink-0">
              <span className="text-xs font-medium text-purple-800">{t('daily_schedule.3.time')}</span>
            </div>
            <div className="flex-grow">
              <h5 className="text-xs font-medium text-gray-800">{t('daily_schedule.3.activity')}</h5>
            </div>
  ...
  const additionalContent = (
    <div className="space-y-3">
      <div className="flex items-start">
        ...
        <div>
          <h4 className="font-medium text-gray-800 text-sm">{t('features.0.title')}</h4>
      ...
      <div className="flex items-start">
        ...
        <div>
          <h4 className="font-medium text-gray-800 text-sm">{t('features.1.title')}</h4>
      ...
      <div className="flex items-start">
        ...
        <div>
          <h4 className="font-medium text-gray-800 text-sm">{t('features.2.title')}</h4>
