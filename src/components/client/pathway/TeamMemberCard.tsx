...
const TeamMemberCard: React.FC<TeamMemberCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Content overlay */}
      <div
        className="absolute bottom-0 left-0 right-0 p-4 transition-all duration-300 z-20"
        style={{
          transform: isHovered
            ? "translateY(0)"
      ...
      >
        ...
        <p
          className={`text-sm transition-colors duration-300 ${
            isHovered ? "text-indigo-200" : "text-indigo-600"
          } font-medium`}
        >
          {role}
        </p>
        ...
        >
          <p className="mt-3 text-sm text-white mb-4">{description}</p>
